...
class ChromiumPlugin(Plugin):
    """Chromium browsers plugin.

    This class contains all History plugins for chromium-based browsers.

    The following chromium-based browsers are supported: Google Chrome, Microsoft Edge.
    When adding a new browser: Create and adjust a <browsername>.py plugin file and adjust this class accordingly.
    """
    ...
    def __init__(self, target: Target) -> None:
        ...
        self.all_users_dirs: list = []
        for dir_list in self.DIRS.values():
            for dir in dir_list:
                self.all_users_dirs.append(dir)
    ...
    def check_compatible(self) -> None:
        ...
        """
        if not len(self._build_userdirs(self.all_users_dirs)):
    ...
    def _iter_db(self, filename: str, browser_name: str) -> Iterator[SQLite3]:
        ...
        """
        browser_dirs: list = []
        # If no browser name is provided, use all browser dirs.
        if not browser_name:
            browser_dirs = self.all_users_dirs
        else:
            browser_dirs = list(self.DIRS[browser_name])

        for user, cur_dir in self._build_userdirs(browser_dirs):
    ...
    def history(self, browser_name: str = None) -> Iterator[ChromiumBrowserHistoryRecord]:
        ...
        Args:
            browser_name: The of the browser a string.
        Yields:
    ...
    def chrome(self):
        ...
        Yields ChromiumBrowserHistoryRecords with the following fields:
            ...
            source: (path): The source file of the history record.
        """
        yield from self.history("chrome")
