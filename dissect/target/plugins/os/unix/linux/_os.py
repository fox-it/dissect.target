...
from dissect.target.helpers.network_managers import LinuxNetworkManager, parse_unix_dhcp_log_messages
...
class LinuxPlugin(UnixPlugin, LinuxNetworkManager):
    ...
    @export(property=True)
    def ips_dhcp(self) -> list[str]:
        """Returns a list of DHCP IP addresses found on the host system."""
        return parse_unix_dhcp_log_messages(self.target)
