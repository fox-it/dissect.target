...
class SafariRecentlyClosedTabsPlugin(Plugin):
    ...
    def check_compatible(self) -> None:
        if not (self.files):
            raise UnsupportedPluginError("No RecentlyClosedTabs.plist found")
    ...
    def safari_recently_closed_tabs(self) -> Iterator[DynamicDescriptor]:
        """Return macOS Safari recently closed tabs information."""
        yield from build_plist_records(self, self.files, function_name="macos/safari_recently_closed_tabs")
