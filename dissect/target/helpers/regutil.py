...
REG_FLEX_HEADER = "Windows Registry Editor Version 5.00"
_REG_LINE_LIMIT = 80
...
class RegistryKey:
    ...
    def __init__(self, hive: RegistryHive | None = None):
        self.hive = hive
...
class VirtualHive(RegistryHive):
    ...
    def map_value(self, path: str, name: str, value: ValueType | RegistryValue) -> None:
...
class VirtualKey(RegistryKey):
    ...
    def __init__(self, hive: RegistryHive, path: str, class_name: str | None = None):
        self._path = path
        if not path.strip("\\"):
            self._name = "VROOT"
        else:
    ...
    def add_subkey(self, name: str, key: str) -> None:
        """Add a subkey to this key."""
        self._subkeys[name.lower()] = key

    def add_value(self, name: str, value: ValueType | RegistryValue) -> None:
...
class HiveCollection(RegistryHive):
    ...
    def __init__(self, hives: list[RegistryHive] | None = None):
        self.hives = hives or []
        super().__init__()
    ...
    def keys(self, keys: list | str) -> Iterator[RegistryKey]:
        keys = [keys] if not isinstance(keys, list) else keys
        for key in keys:
            try:
...
class KeyCollection(RegistryKey):
    ...
    def __init__(self, keys: list[RegistryKey] | None = None):
        self.keys = keys or []
        super().__init__()
    ...
    def __getitem__(self, index: int) -> RegistryValue:
    ...
    def add(self, key: KeyCollection | RegistryKey) -> None:
...
class ValueCollection(RegistryValue):
    ...
    def __init__(self, values: list[RegistryValue] | None = None):
...
class RegfHive(RegistryHive):
    ...
    def __init__(self, filepath: Path, fh: BinaryIO | None = None):
...
class RegFlex:
    ...
    def dump_definition(self, fh: TextIO, paths: list[str] | None = None) -> None:
        """Dump the parsed registry hives to a .reg file format.

        This is the inverse of :meth:`map_definition`. When ``paths`` is
        given, only the listed registry subtrees are written; otherwise every
        hive stored in :attr:`hives` is exported in full.

        The header always contains a comment line indicating that the file was
        generated by RegFlex. When ``paths`` is provided each path is also
        listed as an individual comment line so that the exported file is
        self-documenting.

        Args:
            fh: A file-like object opened in text mode to write the registry export to.
            paths: Optional list of full registry paths to export
                (e.g. ``["HKEY_LOCAL_MACHINE\\\\SOFTWARE\\\\Test"]``).
                When ``None`` all hives are exported in full.
        """
        fh.write(f"{REG_FLEX_HEADER}\n\n")
        fh.write("; This .reg file was generated by RegFlex (part of Dissect Target)\n")
        if paths:
        ...
        if not paths:
            for hive_name, hive in self.hives.items():
                for subkey in hive.root().subkeys():
                    write_reg_key(fh, subkey, f"{hive_name}\\{subkey.name}")
            return

        for path in paths:
            hive_name, _, key_path = path.partition("\\")
            hive_name = hive_name.upper()
            hive = self.hives.get(hive_name)
            if hive is None:
                log.warning("RegFlex: hive %r not found, skipping path %r", hive_name, path)
                continue
            try:
                key = hive.key(key_path) if key_path else hive.root()
            except RegistryKeyNotFoundError:
                log.warning("RegFlex: key %r not found in hive %r, skipping", key_path, hive_name)
                continue
            write_reg_key(fh, key, path)
...
def _escape_reg_str(value: str) -> str:
...
def _format_hex_data(data: bytes, prefix: str) -> str:
    ...
    :data:`_REG_LINE_LIMIT` characters are split with a trailing backslash and
    ...
    for i, pair in enumerate(hex_pairs):
        ...
        fits = len(current) + len(entry) + (0 if is_last else 1) <= _REG_LINE_LIMIT
...
def write_flex_value(value: RegistryValue) -> str:
    """Format a :class:`RegistryValue` as a ``.reg`` file value line.

    This is the inverse of :func:`parse_flex_value`.

    When the registry type is :attr:`RegistryValueType.NONE` the Python type
    ...
    name_str = "@" if name == "" else f'"{_escape_reg_str(name)}"'
    ...
    if vtype == RegistryValueType.MULTI_SZ:
        ...
        return _format_hex_data(raw, f"{name_str}=hex(7):")

    if vtype == RegistryValueType.DWORD_BIG_ENDIAN:
        raw = int(data).to_bytes(4, "big") if isinstance(data, int) else (data or b"")
        return _format_hex_data(raw, f"{name_str}=hex(5):")

    if vtype == RegistryValueType.QWORD:
        raw = int(data).to_bytes(8, "little") if isinstance(data, int) else (data or b"")
        return _format_hex_data(raw, f"{name_str}=hex(b):")

    if vtype == RegistryValueType.NONE:
        raw = data if isinstance(data, bytes) else b""
        return _format_hex_data(raw, f"{name_str}=hex(0):")

    # Fallback for LINK, RESOURCE_LIST, FULL_RESOURCE_DESCRIPTOR, RESOURCE_REQUIREMENTS_LIST, etc.
    raw = data if isinstance(data, bytes) else b""
    return _format_hex_data(raw, f"{name_str}=hex({int(vtype):x}):")


def write_reg_key(fh: TextIO, key: RegistryKey, path: str) -> None:
    """Recursively write a registry key and its subkeys in .reg format.

    Args:
        fh: A file-like object opened in text mode.
        key: The registry key to write.
        path: The full registry path for this key
            (e.g. ``HKEY_LOCAL_MACHINE\\SOFTWARE\\Test``).
    """
    fh.write(f"[{path}]\n")
    fh.writelines(f"{write_flex_value(reg_value)}\n" for reg_value in key.values())
    fh.write("\n")
    for subkey in key.subkeys():
        write_reg_key(fh, subkey, f"{path}\\{subkey.name}")
...
def glob_ext(key_collection: KeyCollection, pattern: str) -> Iterator[KeyCollection]:
    ...
    glob_in_key_path = glob_ext1 if has_glob_magic(last_key) else glob_ext0
