    ...
    with monkeypatch.context() as m:
        ...
        if output_type == "json":
            assert json.loads(stdout) == {
                ...
                "last_activity": None,
                "mounts": [{"fs": "VirtualFilesystem", "path": "/"}],
            }

        elif output_type == "record":
            assert (
                stdout
                == "<target/info hostname='ubuntu' domain=None last_activity=None install_date=2024-07-02 12:00:56+00:00 ips=[net.ipaddress('1.2.3.4')] os_family='linux' os_version='Ubuntu 22.04.4 LTS (Jammy Jellyfish)' architecture=None language=[] timezone='Europe/Amsterdam' disks=[] volumes=[] mounts=[\"{'fs': 'VirtualFilesystem', 'path': '/'}\"] children=[]>\n"  # noqa: E501
            )
