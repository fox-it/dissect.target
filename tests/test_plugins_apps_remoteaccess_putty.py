...
from dissect.target.plugins.apps.remoteaccess.putty import PuTTYPlugin
...
def test_putty_plugin_ssh_host_keys_unix(target_unix_users, fs_unix):
    ...
    assert records[0].source == path.from_posix("/root/.putty/sshhostkeys")
    ...
    assert records[1].source == path.from_posix("/root/.putty/sshhostkeys")

    assert records[2].ts is not None
    ...
    assert records[2].source == path.from_posix("/root/.putty/sshhostkeys")
...
def test_putty_plugin_ssh_host_keys_windows(target_win_users, fs_win, hive_hku):
    ...
    assert records[0].key_type == "ssh-ed25519"
    assert records[0].public_key == "AAAAC3NzaC1lZDI1NTE5AAAAIHUl23i/4p/7xcZnNPDK+Dr+A539zpEEXutrm/tESFYq"
    assert str(records[0].source) == "Software\\SimonTatham\\PuTTY\\SshHostKeys"
...
def test_putty_plugin_saved_sessions_unix(target_unix_users, fs_unix):
    ...
    assert records[0].ts == datetime.utcfromtimestamp(
        stat(absolute_path("data/plugins/remoteaccess/putty/sessions/example-saved-session")).st_mtime
    ...
    assert records[0].source == path.from_posix("/root/.putty/sessions/example-saved-session")
...
def test_putty_plugin_saved_sessions_windows(target_win_users, fs_win, hive_hku):
    ...
    assert records[0].port_forward == ""
    assert records[0].manual_ssh_host_keys == ""
    assert str(records[0].source) == "Software\\SimonTatham\\PuTTY\\Sessions\\example-saved-session"
