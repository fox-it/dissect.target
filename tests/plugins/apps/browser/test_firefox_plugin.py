...
def test_firefox_history(target_win, fs_win, tmp_path, target_win_users):
    __setup(target_win, fs_win, tmp_path, target_win_users, "places.sqlite")
    records = list(target_win.firefox.history())
    assert len(records) == 24


def test_firefox_downloads(target_win, fs_win, tmp_path, target_win_users):
    __setup(target_win, fs_win, tmp_path, target_win_users, "places.sqlite")
    records = list(target_win.firefox.downloads())
    assert len(records) == 3


def test_firefox_cookies(target_win, fs_win, tmp_path, target_win_users):
...
def __setup(target_win, fs_win, tmp_path, target_win_users, db):
