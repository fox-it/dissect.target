...
def test_edge_downloads(target_win, fs_win, tmp_path, target_win_users):
    __setup(target_win, fs_win, tmp_path, target_win_users)
    records = list(target_win.edge.downloads())
    assert len(records) == 2


def test_edge_extensions(target_win, fs_win, tmp_path, target_win_users):
    __setup(target_win, fs_win, tmp_path, target_win_users)
    records = list(target_win.edge.extensions())
    assert len(records) == 39


def test_edge_history(target_win, fs_win, tmp_path, target_win_users):
    __setup(target_win, fs_win, tmp_path, target_win_users)
    records = list(target_win.edge.history())
    assert len(records) == 45


def __setup(target_win, fs_win, tmp_path, target_win_users) -> None:
