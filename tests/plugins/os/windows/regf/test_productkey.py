...
def test_productkey(target_win_users: Target, hive_hklm: VirtualHive) -> None:
    ...
    key.add_value(
        "CacheStore", "C:\\Windows\\System32\\config\\systemprofile\\AppData\\Local\\Microsoft\\Windows\\Cache"
    )
    key.add_value(
        "TokenStore",
        "C:\\Windows\\ServiceProfiles\\NetworkService\\AppData\\Local\\Microsoft\\Windows NT\\SoftwareProtectionPlatform",
    )
    ...
    assert (
        record.cache_store == "C:\\Windows\\System32\\config\\systemprofile\\AppData\\Local\\Microsoft\\Windows\\Cache"
    )
    assert (
        record.token_store
        == "C:\\Windows\\ServiceProfiles\\NetworkService\\AppData\\Local\\Microsoft\\Windows NT\\SoftwareProtectionPlatform"
