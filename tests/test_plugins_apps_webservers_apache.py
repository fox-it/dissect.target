...
def test_plugins_apps_webservers_apache_infer_access_log_citrix_netscaler_combined_response_time():
    log_combined = (
        '127.0.0.1 - - [11/Aug/2023:08:30:00 +0000] [82775] "GET / HTTP/1.1" 200 18705 "-" "curl/7.78.0" "Time: 39770 '
        'microsecs"'
    )
    assert infer_access_log_format(log_combined) == LogFormat.CITRIX_NETSCALER_COMBINED_RESPONSE_TIME


def test_plugins_apps_webservers_apache_infer_access_log_citrix_netscaler_combined_response_time_with_headers():
    log_combined = (
    ...
    assert infer_access_log_format(log_combined) == LogFormat.CITRIX_NETSCALER_COMBINED_RESPONSE_TIME_WITH_HEADERS


def test_plugins_apps_webservers_apache_access_txt(target_unix, fs_unix):
...
def test_plugins_apps_webservers_apache_access_gz(target_unix, fs_unix):
